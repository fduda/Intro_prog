...
def is_first_player_winner(pofn, posn, ptfn, ptsn):
    """
    This function takes four parameters, calculates all the means between the first two and the last two. Then,
    it stores the difference between the mean calculated with the first two numbers and with the last two numbers.
    Does it for all three means. Sums the results, analyses the signal and returns True of False.
    """

    # The parameters name are a written in a shorter way, where "pofn" means player_one_first_number, "posn" means
    # player_one_second_number, "ptfn", means player_two_first_number, and "ptsn" means player_two_second_number.
    # This is abbreviation was made in order to keep the code cleaner and shorter.
